...
class KeywordSHAPVisualizer:
    ...
    def _create_static_beeswarm(self, X, shap_values, feature_names, save_path=None):
        ...
        plt.title('Biology Keyword Contribution to Abundance & Density', fontsize=16, fontweight='bold')
        plt.tight_layout()
        if save_path:
    ...
    def _create_interactive_beeswarm(self, X, shap_values, feature_names, save_path=None):
        ...
        fig = px.scatter(plot_df, x='shap_value', y='feature',
                        ...
                        title='Interactive SHAP: Biology Keywords')
    ...
    def create_abundance_density_heatmap(self, df, save_path=None):
        ...
        plt.title('Biology Keyword Correlation Heatmap', fontsize=16, fontweight='bold')
        plt.tight_layout()
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.show()
...
class SHAPAnalyzerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("SHAP Analyzer - Biology Keywords")
        self.root.geometry("650x380")
        self.visualizer = KeywordSHAPVisualizer()
        self.df = None
        self.shap_vals = None

        ttk.Label(root, text="Biology Keyword SHAP Analyzer", font=('Arial',16,'bold')).pack(pady=15)
    ...
    def show_interactive(self):
        if self.shap_vals is None:
            messagebox.showwarning("Warning","Calculate SHAP first")
            return
        self.visualizer.create_shap_beeswarm_plot(self.df, self.shap_vals, "interactive_bio_shap.html", interactive=True)
